        ...
        <div className="flex items-end justify-between mb-8">
          ...
          <div className="text-right">
            ...
            <div className="w-48 h-2 bg-zinc-800 rounded-full overflow-hidden">
              <div className="h-full bg-red-500 rounded-full" style={{ width: `${(completed / 22) * 100}%` }} />
            </div>
          ...
          {["ALL", "COMPLETED", "SCHEDULED", "CANCELLED"].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-4 py-1.5 rounded-full text-xs font-bold border transition-all ${
                filter === f
                  ? f === "CANCELLED" ? "border-red-500 text-red-400 bg-red-500/10"
                  : f === "COMPLETED" ? "border-green-500 text-green-400 bg-green-500/10"
              ...
              {f !== "ALL" && (
                <span className="ml-1 opacity-60">
                  {f === "COMPLETED" ? completed : f === "CANCELLED" ? cancelled : races.filter(r => r.status === "SCHEDULED").length}
                </span>
              )}
            </button>
          ))}
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filtered.map((race) => {
              ...
              return (
                <div key={race.id}
                  className={`flex items-center justify-between rounded-xl px-6 py-4 border transition-all ${
                    isCancelled
                ...
                >
                  <div className="flex items-center gap-5">
                    <div className="text-center w-8">
                      <p className="text-2xl font-black text-zinc-700">{race.roundNumber}</p>
                    </div>
                    <span className="text-xl">{COUNTRY_FLAGS[race.circuit?.country] || "🏁"}</span>
                    <div>
                      ...
                      {isCancelled && (
                        <p className="text-xs text-red-400/70 mt-1">⚠️ Cancelled — Middle East conflict</p>
                      )}
                    </div>
                  </div>
