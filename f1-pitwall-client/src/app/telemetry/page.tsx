...
function DualSpeedChart({ data1, data2, color1, color2, label }: {
  ...
  useEffect(() => {
    ...
    drawLine(data1, color1, 2);
    drawLine(data2, color2 + "CC", 2);
      ...
      stompClient.connect({}, () => {
        ...
        stompClient.subscribe("/topic/telemetry", (message: TelemetryFrame) => {
          ...
          setSpeedHistory(prev => {
            const next = { ...prev };
            data.forEach(d => { next[d.driverName] = [...(next[d.driverName] || []).slice(-(MAX_HISTORY - 1)), d.speed]; });
            return next;
          });
          setRpmHistory(prev => {
            const next = { ...prev };
            data.forEach(d => { next[d.driverName] = [...(next[d.driverName] || []).slice(-(MAX_HISTORY - 1)), d.rpm]; });
            return next;
          });
          setThrottleHistory(prev => {
            const next = { ...prev };
            data.forEach(d => { next[d.driverName] = [...(next[d.driverName] || []).slice(-(MAX_HISTORY - 1)), d.throttle]; });
            return next;
          });
        });
      }, () => setConnected(false));
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => setMode("single")}
                className={`px-4 py-2 rounded-lg text-xs font-bold border transition-all ${mode === "single" ? "bg-red-500/20 border-red-500 text-red-400" : "border-zinc-700 text-zinc-500 hover:border-zinc-500"}`}>
                SINGLE
              </button>
              <button onClick={() => setMode("compare")}
                className={`px-4 py-2 rounded-lg text-xs font-bold border transition-all ${mode === "compare" ? "bg-blue-500/20 border-blue-500 text-blue-400" : "border-zinc-700 text-zinc-500 hover:border-zinc-500"}`}>
                COMPARE
              </button>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                {mode === "compare" && (
                  <p className="text-xs text-zinc-600">
                    <span style={{ color: selectedDriver?.teamColor }}>■</span> vs
                    <span style={{ color: compareDriver?.teamColor || "#666" }}> ■</span>
                  </p>
                )}
              </div>
              {drivers.map((d) => {
                ...
                return (
                  <div key={d.driverName}
                    onClick={() => {
                      if (mode === "single") {
                        setSelected(d.driverName);
                      } else {
                        if (isSelected) return;
                        setCompareWith(d.driverName);
                      }
                    }}
                    className={`bg-zinc-900 rounded-xl p-4 cursor-pointer transition-all border ${
                      isSelected ? "border-2" : isCompare ? "border-2 border-dashed" : "border-zinc-800 hover:border-zinc-600"
                    }`}
                  ...
                  {!compareDriver ? (
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        {[selectedDriver, compareDriver].map((d, i) => d && (
                          <div key={d.driverName} className="bg-zinc-900 rounded-xl p-4 border-t-2" style={{ borderColor: d.teamColor }}>
                            ...
                            <div className="flex items-center justify-between mt-3">
                              <div className="text-center">
                                <p className="text-2xl font-black text-white">{d.speed.toFixed(0)}</p>
                                <p className="text-xs text-zinc-600">km/h</p>
                              </div>
                              <div className="text-center">
                                <p className="text-lg font-black text-white">G{d.gear}</p>
                                <p className="text-xs text-zinc-600">GEAR</p>
                              </div>
                              <div className="text-center">
                                <p className="text-lg font-black" style={{ color: d.drsActive ? "#22c55e" : "#666" }}>
                                  {d.drsActive ? "ON" : "OFF"}
                                </p>
                                <p className="text-xs text-zinc-600">DRS</p>
                              </div>
                              <div className="text-center">
                                <p className="text-lg font-black text-white">P{d.position}</p>
                                <p className="text-xs text-zinc-600">POS</p>
                              </div>
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="bg-zinc-900 rounded-xl p-5 border border-zinc-800">
                        <div className="flex items-center gap-4 mb-4">
                          ...
                          <div className="flex items-center gap-3">
                            <span className="flex items-center gap-1 text-xs" style={{ color: selectedDriver?.teamColor }}>
                              <span className="w-6 h-0.5 inline-block" style={{ backgroundColor: selectedDriver?.teamColor }} />
                              {selectedDriver?.driverName.split(" ").pop()}
                            </span>
                            <span className="flex items-center gap-1 text-xs" style={{ color: compareDriver.teamColor }}>
                              <span className="w-6 h-0.5 inline-block" style={{ backgroundColor: compareDriver.teamColor }} />
                              {compareDriver.driverName.split(" ").pop()}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4">
                          <DualSpeedChart
                            data1={speedHistory[selectedDriver?.driverName || ""] || []}
                            data2={speedHistory[compareDriver.driverName] || []}
                            color1={selectedDriver?.teamColor || "#fff"}
                            color2={compareDriver.teamColor}
                            label="SPEED (km/h)"
                          />
                          <DualSpeedChart
                            data1={throttleHistory[selectedDriver?.driverName || ""] || []}
                            data2={throttleHistory[compareDriver.driverName] || []}
                            color1={selectedDriver?.teamColor || "#fff"}
                            color2={compareDriver.teamColor}
                            label="THROTTLE (%)"
                          />
                          <DualSpeedChart
                            data1={rpmHistory[selectedDriver?.driverName || ""] || []}
                            data2={rpmHistory[compareDriver.driverName] || []}
                            color1={selectedDriver?.teamColor || "#fff"}
                            color2={compareDriver.teamColor}
                            label="RPM"
                          />
                      ...
                      <div className="bg-zinc-900 rounded-xl p-5 border border-zinc-800">
                        ...
                        <div className="mt-4 pt-4 border-t border-zinc-800 grid grid-cols-2 gap-4">
                          <div className="text-center">
                            <p className="text-xs text-zinc-600 mb-1">GAP TO LEADER</p>
                            <p className="text-xl font-black" style={{ color: selectedDriver?.teamColor }}>
                              +{selectedDriver?.gap.toFixed(3)}s
                            </p>
                          </div>
                          <div className="text-center">
                            <p className="text-xs text-zinc-600 mb-1">GAP TO LEADER</p>
                            <p className="text-xl font-black" style={{ color: compareDriver.teamColor }}>
                              +{compareDriver.gap.toFixed(3)}s
                            </p>
                          </div>
                        </div>
                      </div>
