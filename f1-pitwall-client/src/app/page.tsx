...
function StatCard({ label, value, sub, href, delay }: {
  ...
  const icons: Record<string, string> = { DRIVERS: "🏎", TEAMS: "🏗", RACES: "🏁", CIRCUITS: "🗺" };
  ...
  return (
    <Link href={href}
      ...
      <div className="relative">
        ...
        <p className="text-6xl font-black text-white mb-2 group-hover:text-red-400 transition-colors duration-300 tabular-nums">
          {count}
        </p>
...
export default function Home() {
  ...
  const fetchData = async () => {
    try {
      ...
      setRaces(racesData.filter((r: any) => !r.name.toLowerCase().includes("sprint")).slice(0, 6));
  ...
  const progressPct = (allCompleted / 22) * 100;
  ...
  return (
    <div className="min-h-screen bg-zinc-950 relative overflow-x-hidden">
      <style>{`
        ...
        .slide-right { animation: slideRight 1s ease-out both; }
        .pulse-glow { animation: pulse-glow 3s ease-in-out infinite; }
        .count-flip { animation: countFlip 0.3s ease-out; }
      `}</style>
      ...
      <div className="fixed inset-0 z-0">
        ...
        {/* Diagonal accent */}
        <div className="absolute top-0 right-0 w-px h-full bg-gradient-to-b from-transparent via-red-500/10 to-transparent" style={{ right: "20%" }} />
      </div>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-8 py-10">
        ...
        <div className="mb-12 flex items-start justify-between flex-wrap gap-6">
          ...
          {/* Next Race countdown */}
          {nextRace && (
            <div className="relative bg-zinc-900/80 backdrop-blur border border-zinc-700/50 rounded-2xl px-6 py-5 text-right pulse-glow card-enter overflow-hidden"
        ...
        {/* Season progress */}
        <div className="bg-zinc-900/60 backdrop-blur border border-zinc-800/50 rounded-2xl p-6 mb-8 card-enter overflow-hidden relative"
          style={{ animationDelay: "600ms" }}>
          <div className="absolute inset-0 opacity-[0.02]"
            style={{ backgroundImage: "repeating-linear-gradient(90deg, #ef4444 0, #ef4444 1px, transparent 1px, transparent 40px)" }} />
          <div className="relative flex items-center justify-between mb-4">
            ...
            <p className="text-xs font-mono text-zinc-500">
              <span className="text-white font-bold">{allCompleted}</span> / 22 active races
            </p>
          </div>
          {/* Progress track */}
          <div className="relative h-3 bg-zinc-800 rounded-full overflow-hidden mb-4">
            ...
            />
            {/* Shimmer */}
            <div className="absolute inset-y-0 left-0 w-full overflow-hidden rounded-full" style={{ width: `${progressPct}%` }}>
          ...
          <div className="relative h-1 mb-3">
            {Array.from({ length: 22 }).map((_, i) => (
              <div
                ...
                style={{
                  left: `${(i / 22) * 100}%`,
                  backgroundColor: i < allCompleted ? "#ef4444" : "#3f3f46",
                }}
              />
            ))}
          </div>
          <div className="flex gap-5">
            ...
            <span className="flex items-center gap-1.5 text-xs text-zinc-500">
              ...
              {22 - allCompleted - cancelled} Scheduled
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Race Calendar */}
          <div className="bg-zinc-900/60 backdrop-blur border border-zinc-800/50 rounded-2xl p-6 card-enter"
            ...
            <div className="space-y-1">
              {loading ? (
                ...
                return (
                  <div key={race.id}
                    className={`flex items-center justify-between py-2.5 px-3 rounded-xl transition-all duration-200 hover:bg-zinc-800/40 group ${isCancelled ? "opacity-40" : ""}`}
                      ...
                      <div className="min-w-0">
                        ...
                        {winner && (
                          <p className="text-xs text-zinc-600 mt-0.5 truncate">
                            🏆 <span className="text-zinc-400">{winner.driver}</span>
                          </p>
                        )}
          ...
          {/* Race Weekend Widget */}
          <div className="card-enter" style={{ animationDelay: "800ms" }}>
            <RaceWeekendWidget />
          </div>
          ...
          <div className="bg-zinc-900/60 backdrop-blur border border-zinc-800/50 rounded-2xl p-6 card-enter"
            ...
            <div className="space-y-1">
              {loading ? (
                <div className="space-y-2">
                  {[1, 2, 3, 4, 5].map(i => <div key={i} className="h-14 bg-zinc-800/50 rounded-lg animate-pulse" />)}
                </div>
              ) : topDrivers.map((driver, i) => (
                <div key={driver.id}
                  className="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-zinc-800/40 transition-all duration-200 group">
                  ...
                  <div
                    className="w-0.5 h-10 rounded-full flex-shrink-0 transition-all duration-300 group-hover:h-12"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs mt-0.5 truncate font-medium" style={{ color: driver.team?.colorHex || "#666" }}>
                      {driver.team?.name}
                    </p>
                  </div>
