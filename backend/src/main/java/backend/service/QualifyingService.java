...
public class QualifyingService {
    ...
    private Integer findQualifyingSession(String country, int season, boolean isSprint) {
        try {
            ...
            String url = OPENF1_BASE + "/sessions?year=" + season + "&session_name=" + sessionType.replace(" ", "+");
    ...
    public boolean syncFromSession(Integer sessionKey, Race race) {
        try {
            ...
            sleep(400);
            Map<Integer, Double> q1Times = fetchBestLapTimes(sessionKey, 1);
            sleep(400);
            Map<Integer, Double> q2Times = fetchBestLapTimes(sessionKey, 2);
            sleep(400);
            Map<Integer, Double> q3Times = fetchBestLapTimes(sessionKey, 3);
            ...
            List<Map.Entry<Integer, Double>> gridOrder = buildGridOrder(q1Times, q2Times, q3Times, driverInfoMap.keySet());

            List<QualifyingResult> results = new ArrayList<>();
            for (int i = 0; i < gridOrder.size(); i++) {
                Integer driverNum = gridOrder.get(i).getKey();
                ...
                int gridPos = i + 1;
                Double q1 = q1Times.get(driverNum);
                Double q2 = q2Times.get(driverNum);
                Double q3 = q3Times.get(driverNum);
                Double best = q3 != null ? q3 : (q2 != null ? q2 : q1);

                boolean elQ1 = q2 == null && q3 == null && q1 != null;
            ...
            log.info("✅ [Qualifying] Synced {} — {} drivers", race.getName(), results.size());
            return true;
    ...
    private Map<Integer, Double> fetchBestLapTimes(Integer sessionKey, int segment) {
        Map<Integer, Double> bestTimes = new HashMap<>();
        try {
            String url = OPENF1_BASE + "/laps?session_key=" + sessionKey + "&segment_sector_1=" + segment;
            ...
            log.debug("[Qualifying] Lap fetch error segment {}: {}", segment, e.getMessage());
        }
        return bestTimes;
    }

    private List<Map.Entry<Integer, Double>> buildGridOrder(
            Map<Integer, Double> q1, Map<Integer, Double> q2, Map<Integer, Double> q3,
            Set<Integer> allDrivers) {

        List<Map.Entry<Integer, Double>> q3Entries = new ArrayList<>(q3.entrySet());
        q3Entries.sort(Map.Entry.comparingByValue());

        List<Map.Entry<Integer, Double>> q2Entries = q2.entrySet().stream()
            .filter(e -> !q3.containsKey(e.getKey()))
            .sorted(Map.Entry.comparingByValue())
            .collect(Collectors.toList());

        List<Map.Entry<Integer, Double>> q1Entries = q1.entrySet().stream()
            .filter(e -> !q2.containsKey(e.getKey()) && !q3.containsKey(e.getKey()))
            .sorted(Map.Entry.comparingByValue())
            .collect(Collectors.toList());

        List<Map.Entry<Integer, Double>> grid = new ArrayList<>();
        grid.addAll(q3Entries);
        grid.addAll(q2Entries);
        grid.addAll(q1Entries);

        for (Integer driverNum : allDrivers) {
            boolean found = grid.stream().anyMatch(e -> e.getKey().equals(driverNum));
            if (!found) {
                grid.add(Map.entry(driverNum, 999.0));
            }
        }
    ...
    private boolean matchCountry(String openf1Country, String raceCountry) {
        ...
        return c1.contains(c2) || c2.contains(c1) ||
            (c1.contains("united states") && c2.contains("miami")) ||
            (c1.contains("united states") && c2.contains("united states")) ||
    ...
    private Optional<Driver> findDriver(String fullName, Integer carNumber) {
        return driverRepo.findAll().stream()
            .filter(d -> {
                ...
                return dName.equals(fName) || dName.contains(fLast) || fName.contains(dName.split(" ")[dName.split(" ").length - 1]) ||
